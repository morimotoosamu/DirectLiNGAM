...
# 先に実装した総合効果算出関数を利用
source("estimate_total_effect2.r")
...
get_causal_direction_counts <- function(result,
  ...
  results_list <- lapply(unique_keys, function(key) {
    ...
    row <- data.frame(
      ...
      sd_effect = if (length(effects) > 1) sd(effects) else 0,
      ci_lower = quantile(effects, 0.025, names = FALSE),
      ci_upper = quantile(effects, 0.975, names = FALSE),
...
get_directed_acyclic_graph_counts <- function(result,
  ...
  #
  #   if (split_by_causal_effect_sign) {
  #     sign_mat <- matrix(0L, nrow = nrow(am), ncol = ncol(am))
  #     idx <- which(abs(am) > min_causal_effect, arr.ind = TRUE)
  #     if (nrow(idx) > 0) {
  #       for (k in seq_len(nrow(idx))) {
  #         sign_mat[idx[k, 1], idx[k, 2]] <- as.integer(sign(am[idx[k, 1], idx[k, 2]]))
  #       }
  #     }
  #     dag_keys[s] <- paste(as.vector(sign_mat), collapse = ",")
  #     dag_list[[s]] <- sign_mat
  #   } else {
  #     bin_mat <- abs(am) > min_causal_effect
  #     dag_keys[s] <- paste(as.integer(as.vector(bin_mat)), collapse = ",")
  #     dag_list[[s]] <- bin_mat
  #   }
  # }

  # --- 【改善】forループによる文字列変換をapplyで一括処理 ---
  if (split_by_causal_effect_sign) {
    # 符号を抽出して文字列化
    ...
    # モードをintegerにしてからpaste（現在のコードの as.integer(as.vector(...)) と同等）
...
get_probabilities <- function(result, min_causal_effect = NULL) {
  ...
  # n_sampling  <- dim(am_array)[1]
  # n_features  <- dim(am_array)[2]
  # bp <- matrix(0, nrow = n_features, ncol = n_features)
  # for (s in seq_len(n_sampling)) {
  #   bp <- bp + ifelse(abs(am_array[s, , ]) > min_causal_effect, 1, 0)
  # }
  # bp <- bp / n_sampling

  # --- 【改善】forループを廃止し、3次元配列の次元(2, 3)に対して平均を一括計算 ---
  # am_array は [サンプリング数, 行, 列] なので、行と列 (c(2, 3)) ごとに mean をとる
...
get_total_causal_effects <- function(result, min_causal_effect = NULL) {
  ...
  # n_sampling  <- dim(te_array)[1]
  # n_features  <- dim(te_array)[2]
  #
  # # 確率の計算
  # probs <- matrix(0, nrow = n_features, ncol = n_features)
  # for (s in seq_len(n_sampling)) {
  #   probs <- probs + ifelse(abs(te_array[s, , ]) > min_causal_effect, 1, 0)
  # }
  # probs <- probs / n_sampling

  # --- 【改善1】確率の計算を apply で一括処理 ---
  # サンプリング次元（第1次元）を潰して、行・列（第2, 3次元）ごとに
  # 閾値を超える割合(mean)を計算
  ...
  # prob_vec <- sapply(seq_len(nrow(idx)), function(k) probs[idx[k, 1], idx[k, 2]])

  # --- 【改善2】行列インデックスによる一括抽出 ---
  # sapplyを使ったループ抽出ではなく、idx行列をそのまま渡して一発で値を取り出す
  prob_vec <- probs[idx]

  # 非ゼロの中央値を計算
  # effect_vec <- sapply(seq_len(nrow(idx)), function(k) {
  #   vals <- te_array[, idx[k, 1], idx[k, 2]]
  #   nonzero <- vals[abs(vals) > 0]
  #   if (length(nonzero) == 0) return(0)
  #   median(nonzero)
  # })

  # --- 【改善3】非ゼロの中央値を apply で一括処理 ---
  # 全てのペアに対して、非ゼロ要素の中央値を一気に計算
...
get_paths <- function(result, from_index, to_index, min_causal_effect = NULL) {
  ...
  effects_median <- sapply(path_strs, function(ps) {
    median(effects_list[paths_str_list == ps])
  })
